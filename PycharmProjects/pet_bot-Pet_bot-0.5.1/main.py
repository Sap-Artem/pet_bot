...
def start(message):
    if message.from_user.last_name is not None:
        ...
        first_mess = f"{message.from_user.first_name}, здраствуйте,\n" \
                     f"В нашем телеграм-боте вы сможете найти подходящие вам идеи pet-проектов. Вне зависимости от вашего уровня программирования, вы точно не уйдёте с пустыми руками\n" \
                     f"Прежде чем начать, пожалуйста, выберите категорию поиска"
...
def call_query(call):
    ...
    if call.message:
        ...
        if call.data == 'resume_message':
            ...
            #print(admin_list)
            try:
                string = "🔘<b><u>Название проекта:</u></b> " + str(admin_list[1]) + "\n" + \
                         "🔘<b><u>Краткое описание:</u></b> " + str(admin_list[2]) + "\n" + \
                         "🔘<b><u>Описание:</u></b> " + str(admin_list[3]) + "\n" + \
                         "🔘<b><u>Направление:</u></b> " + str(admin_list[5]) + "\n" + \
                         "🔘<b><u>Количество участников:</u></b> " + str(admin_list[7]) + "\n" + \
                         "🔘<b><u>Сроки реализации:</u></b> " + str(admin_list[8]) + "\n" + \
                         "🔘<b><u>Язык программирования:</u></b> " + str(admin_list[6]) + "\n" + \
                         "🔘<b><u>Сложность:</u></b> " + str(admin_list[9]) + "\n" + \
                         "🔘<b><u>Предлагаемые технологии:</u></b> " + str(admin_list[10]) + "\n"
                ...
                back = types.InlineKeyboardButton(text='В главное меню', callback_data='back_message')
                db.row(back)
                bot.send_message(call.message.chat.id, "Больше идей от пользователей нет. Спасибо за работу",
                                 reply_markup=db)
        if call.data == 'back_message':
            if call.message.from_user.last_name is not None:
                first_mess = f"{call.message.from_user.first_name} {call.message.from_user.last_name}, здравствуйте,\n" \
                             f"В нашем телеграм-боте вы сможете найти подходящие вам идеи pet-проектов. Вне зависимости от вашего уровня программирования, вы точно не уйдёте с пустыми руками\n" \
                             f"Прежде чем начать, пожалуйста, выберите категорию поиска"
            else:
                first_mess = f"{call.message.from_user.first_name}, здравствуйте,\n" \
                             f"В нашем телеграм-боте вы сможете найти подходящие вам идеи pet-проектов. Вне зависимости от вашего уровня программирования, вы точно не уйдёте с пустыми руками\n" \
                             f"Прежде чем начать, пожалуйста, выберите категорию поиска"
            position = 1
...
def call_message(message):
    if message.text == 'Язык программирования':
        ...
        if not list:
            bot.send_message(message.chat.id, "Не найдено ни одного pet-проекта. Попробуйте изменить параметры поиcка.")
        ...
        if message.from_user.last_name is not None:
            first_mess = f"{message.from_user.first_name} {message.from_user.last_name}, здравствуйте,\n" \
                     f"В нашем телеграм-боте вы сможете найти подходящие вам идеи pet-проектов. Вне зависимости от вашего уровня программирования, вы точно не уйдёте с пустыми руками\n" \
                     f"Прежде чем начать, пожалуйста, выберите категорию поиска"
        else:
            first_mess = f"{message.from_user.first_name}, здравствуйте,\n" \
                         f"В нашем телеграм-боте вы сможете найти подходящие вам идеи pet-проектов. Вне зависимости от вашего уровня программирования, вы точно не уйдёте с пустыми руками\n" \
                         f"Прежде чем начать, пожалуйста, выберите категорию поиска"
        bot.send_message(message.chat.id, first_mess, reply_markup=main_menu())
    else:
...
def admin(message):
    ...
    try:
        if (db.is_admin(int(str(message.text)))) & (suggest_id == 0):
            ...
            try:
                string = "🔘<b><u>Название проекта:</u></b> " + str(admin_list[1]) + "\n" + \
                     "🔘<b><u>Краткое описание:</u></b> " + str(admin_list[4]) + "\n" + \
                     "🔘<b><u>Описание:</u></b> " + str(admin_list[3]) + "\n" + \
                     "🔘<b><u>Направление:</u></b> " + str(db.get_theme(admin_list[5])) + "\n" + \
                     "🔘<b><u>Количество участников:</u></b> " + str(db.get_people(admin_list[7])) + "\n" + \
                     "🔘<b><u>Сроки реализации:</u></b> " + str(db.get_time(admin_list[8])) + "\n" + \
                     "🔘<b><u>Язык программирования:</u></b> " + str(db.get_language(admin_list[6])) + "\n" + \
                     "🔘<b><u>Сложность:</u></b> " + str(db.get_level(admin_list[9])) + "\n" + \
                     "🔘<b><u>Предлагаемые технологии:</u></b> " + str(admin_list[10])
                suggest_id = admin_list[0]
                db = types.InlineKeyboardMarkup()
                yes = types.InlineKeyboardButton(text='Принять', callback_data='yes_suggest_message')
                ...
                back = types.InlineKeyboardButton(text='В главное меню', callback_data='back_message')
                db.row(back)
                bot.send_message(message.chat.id, "Больше идей от пользователей нет. Спасибо за работу", reply_markup=db)
        else:
            if suggest_id == 0:
                #print("пошёл нафиг")
        ...
        if suggest_id == 0:
            ...
            bot.send_message(message.chat.id, "Ожидается ввод целого числа")
...
def suggest_message(message):
    ...
    if message.text == 'Назад':
        ...
        l1 = types.KeyboardButton(text='начинающий')
        l2 = types.KeyboardButton(text='средний')
        level.add(l1, l2)
        l3 = types.KeyboardButton(text='продвинутый')
        back = types.KeyboardButton(text='Назад')
        level.add(l3, back)
        bot.send_message(message.chat.id, "Введите предполагаемый уровень подготовки (воспользуйтесь шаблонами)", reply_markup=level)
        suggest_position = suggest_position + 1
        ...
        bot.send_message(message.chat.id,
                         "<b><u>Проверьте введённую информацию</u></b>\n" +
                         "🔘<b><u>Название идеи:</u></b> " + suggest_list[0] + "\n" +
                         "🔘<b><u>Краткое описание:</u></b> " + suggest_list[1] + "\n" +
                         "🔘<b><u>Полное описание:</u></b> " + suggest_list[2] + "\n" +
                         "🔘<b><u>" + suggest_list[3] + "</u></b>" + "\n" +
                         "🔘<b><u>Количество участников:</u></b> " + suggest_list[4] + "\n" +
                         "🔘<b><u>" + suggest_list[5] + "</u></b>" + "\n" +
                         "🔘<b><u>" + suggest_list[6] + "</u></b>" + "\n" +
                         "🔘<b><u>Предлагаемые технологии:</u></b> " + suggest_list[7] + "\n" +
                         "🔘<b><u>Уровень подготовки:</u></b> " + suggest_list[8], reply_markup=conf, parse_mode="html")
        suggest_position = suggest_position + 1
