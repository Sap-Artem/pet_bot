...
def call_message(message):
    if message.text == 'Язык программирования':
        ...
        db = types.InlineKeyboardMarkup()
        for data in list:
            project = types.InlineKeyboardButton(text=data[1] + " - " + data[2], callback_data='project_' + str(data[0]))
            db.row(project)
        left = types.InlineKeyboardButton(text='<', callback_data='left_message')
        pos = types.InlineKeyboardButton(text='1/1', callback_data='pos_message')
        right = types.InlineKeyboardButton(text='>', callback_data='right_message')
        db.row(left, pos, right)
        back = types.InlineKeyboardButton(text='В главное меню', callback_data='back_message')
        db.row(back)
        bot.send_message(message.chat.id, text="Все pet-проекты загруженные до " + time + " " + day + "." + month + "." + years, parse_mode='Markdown', reply_markup=db)
